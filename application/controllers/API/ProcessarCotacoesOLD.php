...
class ProcessarCotacoesOLD extends CI_Controller
{
    private $db1;
    private $db2;
    ...
    public function validaCotacao($cd_cotacao, $log, $warning = NULL)
    {
        ...
        if (!IS_NULL($warning)) {
            ...
            $logWarning = fopen("{$dir}warning/{$cd_cotacao}.json", "w+");

            $logs = fopen("{$dir}logs/NOT-ENV_{$cd_cotacao}.json", "w+");
...
//  var_dump($fornecedores); exit();
        ...
        foreach ($fornecedores as $fornecedor) {
            ...
            //    print_r("Processando a Cotacao CD: {$item['cd_cotacao']}" . PHP_EOL);

                ($this->verificaCotacaoEnviada($item['cd_cotacao'])) ? $warning["cotacao_atendida"] = "A cotacao {$item['cd_cotacao']} ja foi atendida !" : '';
              ...
              //  var_dump($produtos); exit();
                ...
                if (!empty($cliente)) {
                    ...
                    (isset($cliente['estado']) && !empty($cliente['estado'])) ?
                        $estado = $this->estado->find("id", "uf = '{$cliente['estado']}'", true) : $warning["estado"] = "Estado nao localizado !";
                    ...
                    $warning = [
                        "cliente" => "Cliente nao localizado na Base de Dados !",
                    ];
                }
                ...
                if ($cotacaoAutomatica) {
                    ...
                    $warning["cotacao_automatica"] =
                        "Cotacao automatica nao habilitada: Fornecedor: {$fornecedor['razao_social']}, Cliente: {$cliente['razao_social']}, Estado: {$cliente['estado']}";
                ...
                /*    if (!isset($forma_pagamento) || empty($forma_pagamento)) {
                        $warning["forma_pagamento"] = "E necessario configurar uma forma de pagamento valida, em regras de vendas -> formas de pagamento";
                        $log["forma_pagamento"] = false;
                    } */

                if (!isset($valor_minimo) || empty($valor_minimo)) {
                    $warning["valor_minimo"] = "E necessario configurar um valor minimo, em regras de vendas -> valor minimo";
                }

                if (!isset($prazo_entrega) || empty($prazo_entrega)) {
                    $warning["prazo_entrega"] = "E necessario configurar prazo de entregas, em regras de vendas -> prazo de entregas";
                ...
                if (isset($warning) && !empty($warning)) {
                    ...
                    $this->validaCotacao($item['cd_cotacao'], $log, $warning);
                ...
                //    var_dump($ids_sintese); exit();

                    if (empty($ids_sintese) || IS_NULL($ids_sintese)) {

                        $warning["ids_sintese"] = "Nenhum Ids_sintese localizado";
                   ...
                   // var_dump($log); exit();
                        ...
                        foreach ($encontrados as $key => $pp) {
                            foreach ($pp as $k => $ppp) {
                                ...
                                if (isset($restrições) && !empty($restrições)) {
                                    ...
                                    $warning["restricoes"] = "Existem restricoes para o produto {$ppp['id_produto']} no fornecedor {$fornecedor['id']} no estado {$estado['id']} !";
                    ...
                    if (isset($warning) && !empty($warning)) {
                        ...
                        $xml = fopen("public/{$item['cd_cotacao']}.xml", "w+");
