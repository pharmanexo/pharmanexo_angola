...
class Consolidacao extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->views = "admin/apoio/de_para/";
        $this->route = base_url("admin/apoio/catalogo/consolidacao/");
    ...
    /**
     * Verifica se existe a sessão id_cliente)apoio
     * Redireciona para a tela de compradores caso o usuario tente acessar diretamente a tela de depara da apoio
     */
    public function getIdClient()
    {
        if ($this->session->has_userdata('id_cliente_apoio')) {

            return $this->session->id_cliente_apoio;
        } else {

            $this->session->set_userdata('warning', ['type' => 'warning', 'message' => 'É necessário escolher um cliente em Bionexo -> clientes']);

            redirect(base_url("/admin/apoio/clientes"));
        }
    }
        ...
        $data['heading'] = $this->template->heading([
            ...
            'buttons' => [
                [
                    'type' => 'a',
                    'id' => 'btnBack',
                    'url' => base_url("admin/apoio/clientes"),
                    'class' => 'btn-secondary',
                    'icone' => 'fa-arrow-left',
    ...
    public function make($id_cliente, $codigo)
    {
        if ($this->input->method() == 'post') {
            ...
            $produto = $this->apoio->where('codigo', $codigo)->where_in('id_cliente', $id_cliente)->get('catalogo')->row_array();
            ...
            foreach ($post['produtos'] as $id_sintese) {
                ...
                $old = $this->pfs->find("*", "id_produto_sintese = {$id_sintese} and cd_produto = '{$codigo}' and id_integrador = '3' and id_cliente = {$id_cliente}", true);
    ...
    public function match($id_cliente, $id_pf)
    {
        ...
        $dados = $this->apoio->where('codigo', $id_pf)->where('id_cliente', $id_cliente)->get('catalogo')->row_array();
     ...
     *
     * @param Int ID do produto no catalogo apoio
     * @return  view
     */
    ...
    /**
     * Oculta/desoculta um produto do catalogo apoio
     *
     * @param Int ID do produto no catalogo apoio
    ...
    public function ocultarProduto($id, $ativar = null)
    {
        ...
        $updt = $this->apoio->where('id', $id)->update('catalogo', ['ocultar' => $ocultar]);
    ...
    public function desocultar_multiple()
    {
        if ($this->input->method() == 'post') {
            ...
            foreach ($post['el'] as $row) {
                ...
                $this->apoio->where("id_cliente", $row['id_cliente'])->where("codigo", $row['codigo'])->update('catalogo', ['ocultar' => 0]);
            }
    ...
    public function to_datatables_ocultados($id_cliente)
    {
        $r = $this->datatable->exec(
            $this->input->get(),
            'vw_produtos_clientes_sem_depara_apoio',
            [
    ...
    public function to_datatables_consolidados($id_cliente)
    {
        $r = $this->datatable->exec(
            ...
            [
                ...
                ['db' => 'produto_bionexo', 'dt' => 'produto_apoio'],
                ['db' => 'produto_comprador', 'dt' => 'produto_comprador', 'formatter' => function ($d, $t) {
                    return $d . "<br> <small>Origem: {$t['produto_apoio']}</small>";
                }],

            ],
            null,
            "id_cliente = {$id_cliente} and integrador = 3"
        );
    ...
    public function saveProductSintese($id_catalogo)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $pc = $this->apoio->where('id', $id_catalogo)->get('catalogo')->row_array();
