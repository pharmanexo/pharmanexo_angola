...
class InteligenciaArtificial extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->route = base_url('/fornecedor/BI/inteligenciaArtificial');
        $this->views = 'fornecedor/BI/inteligenciaArtificial';
    ...
    public function datatableRelatorioProdutosComprador()
    {
        ...
        $data = $this->BI->produtosPrecoIA($dados);
    ...
    public function consulta()
    {
        ...
        if ( $this->input->is_ajax_request() ) {
            ...
            if ($this->form_validation->run() === false) {
                ...
                # Se não existir no POST id_fornecedor ou se é o mesmo do usuario logado
                if ( intval($post['id_fornecedor']) == $this->session->id_fornecedor ) {
                ...
                $preco_catalogo = $this->price->getPrice([
                    'id_fornecedor' => $id_fornecedor,
                    'codigo' => $post['codigo'],
                    'id_estado' => $id_estado
                ]);

                if ( isset($preco_catalogo) && floatval($preco_catalogo) != 0 ) {

                    $variacao = $preco_catalogo * ( (floatval($post['media']) - $preco_catalogo) / ($preco_catalogo * 100) );


                    $preco_catalogo = number_format($preco_catalogo, 2, ',', '.');
                    $variacao = ($variacao > 0 && $variacao < 1) ? number_format( $variacao, 4, ',', '.') : number_format( $variacao, 2, ',', '.');
                    $valorMinimo = number_format( floatval($post['min']), 2, ',', '.' );
                    $valorMaximo = number_format( floatval($post['max']), 2, ',', '.' );
                    $media = number_format( floatval($post['media']), 2, ',', '.' );
                    ...
                    $data = [
                        "O preço de tabela é de R$ {$preco_catalogo}",
                        "O valor minimo é de R$ {$valorMinimo}",
                        "o valor máximo é de R$ {$valorMaximo}",
                    ...
                    $output = [ 'type' => 'warning', 'message' => ['Não foi encontrado preço para este produto no catálogo'] ];
                }                
            }
    ...
    public function exportarPDF()
    {
        ...
        $dados = $this->BI->produtosPrecoIA($filtros);
