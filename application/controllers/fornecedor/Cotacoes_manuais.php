...
{
    private $route, $views, $oncoprod, $DB_COTACAO;
    ...
    /**
     * Exibe a view fornecedor/cotacoes/cotacoes_produtos/main.php
     *
     * @return  view
     */
    public function index()
    {
        $page_title = 'Lista de Cotações Manuais';
    ...
    /**
     * Exibe a view fornecedor/cotacoes/cotacoes_produtos/detail.php
     *
    ...
    /**
     * Obtem dados para os datatables de Cotação de produtos e produtos da cotação
     *
     * @param int $id
     * @param int $submetido
     * @return  json
    ...
    public function getDataSource($cd_cotacao = null)
    {
        if (isset($cd_cotacao)) {
            $data = $this->datatable->exec(
                ...
                "nivel = 1 AND 
                cotacoes_produtos.id_fornecedor = {$this->session->id_fornecedor} AND 
                cotacoes_produtos.submetido = 1 AND 
                cotacoes_produtos.cd_cotacao = '{$cd_cotacao}'"
            );
        } else {
            ...
            $data = $this->datatable->exec(
                ...
                "cp.nivel = 1 and cp.submetido = 1 and cp.id_fornecedor = {$this->session->id_fornecedor}",
                'cd_cotacao'
            );
        }
    ...
    /**
     * Gerar PDF da cotação ou exibi-la via ajax
     *
     * @param string $cd_cotacao
     * @param int chave para determinar retorno da função
