...
class Cotacoes_oncoexo extends MY_Controller
{
    private $urlCliente;
    private $host;
    private $client;
    ...
    public function __construct()
    {
        ...
        $this->route = base_url('/fornecedor/cotacoes_oncoexo/');
        $this->views = 'fornecedor/cotacoes_oncoexo/';
        ...
        if ($this->input->method() == 'post') {
            ...
            if ($regras_venda['type'] == 'warning') {
                ...
                if (isset($rascunho) && $rascunho == 2) {
                    ...
                    redirect("{$this->route}detalhes/{$cd_cotacao}");
                }
            } else {
                        ...
                        'heading' => $this->template->heading([
                            ...
                            'buttons' => [
                                [
                                    'type' => 'a',
                                    'id' => 'btnBack',
                                    'url' => base_url('/fornecedor/cotacoes_oncoexo') . "?uf={$cliente['estado']}",
                                    'class' => 'btn-secondary',
                                    'icone' => 'fa-arrow-left',
    ...
    public function detalhes($cd_cotacao)
    {
        ...
        # Select de fornecedores ONCOEXO
        ...
        foreach ($cotacoes as $cotacao) {
            ...
            if (!empty($cliente)) {
                $cotacaoArray = [
                    ...
                    "link" => "{$this->route}detalhes/{$cotacao['cd_cotacao']}",
    ...
    public function getDetailsProducts($produtos, $id_cliente, $id_estado)
    {
        ...
        # Lista das filiais da ONCOEXO
        $fornecedores_oncoexo = $this->fornecedores->find("*", "id in (" . implode(',', $this->oncoexo) . ")");
        ...
        foreach ($produtos as $kk => $produto) {
            ...
            if (isset($produto['encontrados']) && !empty($produto['encontrados'])) {
                ...
                foreach ($produto['encontrados'] as $k => $p) {
                    ...
                    # Lista de estoques da ONCOEXO
                    foreach ($fornecedores_oncoexo as $f => $fornecedor) {
                    ...
                    if ($produtos[$kk]['encontrados'][$k]['estoque'] < 1) {
                        ...
                        # Add todos os produtos que nao tem estoque
        ...
        foreach ($produtos_ordenados as $id_fornecedor => $produtos_sintese) {
            ...
            foreach ($produtos_sintese['produtos'] as $k => $produto) {
                ...
                foreach ($produto['marcas'] as $p) {
                    ...
                    $ds_marca = $dom->createElement("Ds_Marca", $p['marca']);
    ...
    public function createMirror($produtos_ordenados, $info)
    {
        ...
        foreach ($produtos_ordenados as $kk => $produtos) {
            ...
            foreach ($produtos['produtos'] as $produto) {
                ...
                # Verifica se dentro das marcas do produto existe alguma marca que não esteja excluida
            ...
            $data = "
                <small>
                    <p>
                        ...
                        <label style='margin-right: 20px; font-size: 12px'><strong>Empresa:</strong> {$info['cliente']['razao_social']}</label><br>
                    ...
                    ELSE
                        (select max(pp2.data_criacao)
                            from pharmanexo.produtos_preco pp2
                              where pp2.id_fornecedor = pp.id_fornecedor
                                and pp2.codigo = pp.codigo
                                and pp2.id_estado = pp.id_estado) END) LIMIT 1) preco_unitario
    ...
    /**
     * Obtem os fornecedores ONCOEXO que possuem registro da cotação para utilizar no select
     *
     * @param String codigo da cotação
     * @return array
     */
    ...
    /**
     * Atualiza campo ocultar em cotacoes para não ser exibida novamente na listagem da ONCOEXO
     *
     * @param - String - codigo da cotação
     * @param - int - ID do fornecedor
     * @return  json
    ...
    public function combinar_produto_marca()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($this->oncoexo as $fornecedor) {
