...
class Estudos extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->route = base_url("/fornecedor/arquivos/estudos");
    ...
    {
        $page_title = "Estudos de Produtos";
        ...
        foreach ($lista as $k => $item) {
            $lista[$k]['link'] = base_url(PUBLIC_PATH . "arquivos/{$this->session->id_fornecedor}/estudos/{$item['link']}");
        }

        $data['lista'] = $lista;

        $this->load->view("{$this->views}/main_list_estudos", $data);
    }
