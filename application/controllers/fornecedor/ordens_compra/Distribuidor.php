...
class Distribuidor extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->views = 'fornecedor/ordens_compra/distribuidor';
        $this->route = base_url('fornecedor/ordens_compra/distribuidor');
    ...
    private function main()
    {
        ...
        $page_title = "Ordens de Compra Dist x Dist";
    ...
    public function detalhes($idOC)
    {
        ...
        $data['oc'] = $this->db
            ->select('oc.*, f.nome_fantasia, f.cnpj, f.estado, f.razao_social, f.email, f.telefone, st.descricao as status, fp.descricao as fp, pz.prazo as prazo')
            ->from('b2b_ocs oc')
            ->join('fornecedores f', 'f.id = oc.id_comprador', 'LEFT')
            ->join('b2b_ocs_situacao st', 'st.id = oc.situacao', 'LEFT')
            ->join('prazos_entrega pz', 'pz.id = oc.prazo_entrega', 'LEFT')
            ->join('formas_pagamento_fornecedores fpf', 'fpf.id = oc.id_forma_pagamento', 'LEFT')
            ->join('formas_pagamento fp', 'fp.id = fpf.id_forma_pagamento', 'LEFT')
            ->where('oc.id', $idOC)->get()->row_array();
       ...
       /* if (empty($data['oc'])) {
        ...
        $produtos = $this->db
            ->select('prd.*, pc.nome_comercial, pc.apresentacao, pc.marca, pc.quantidade_unidade, pc.unidade')
            ->from('b2b_ocs_produtos prd')
            ->join('produtos_catalogo pc', "pc.codigo = prd.codigo and pc.id_fornecedor = {$data['oc']['id_fornecedor']}")
            ->where('prd.id_ordem_compra', $idOC)
            ->get()
            ->result_array();

       foreach ($produtos as $k => $produto){
           $produtos[$k]['preco_caixa'] = ($produto['preco'] * $produto['quantidade_unidade']);
       }

       $data['oc']['produtos'] = $produtos;
    ...
    public function to_datatable()
    {
        $r = $this->datatable->exec(
            ...
            [
                ['db' => 'ocs.id', 'dt' => 'id'],
                ['db' => 'ocs.id_solicitacao', 'dt' => 'id_solicitacao'],
                ['db' => 'ocs.situacao', 'dt' => 'situacao'],
                ['db' => 'ocs.data_criacao', 'dt' => 'data_criacao'],
                ['db' => 'f.nome_fantasia', 'dt' => 'nome_fantasia'],
                ['db' => 'f.estado', 'dt' => 'estado'],
                ['db' => 'sts.descricao', 'dt' => 'descricao'],
            ],
            [
                ['fornecedores f', 'f.id = ocs.id_comprador'],
                ['b2b_ocs_situacao sts', 'sts.id = ocs.situacao'],
            ],
            'ocs.id_fornecedor = ' . $this->session->id_fornecedor
        );
