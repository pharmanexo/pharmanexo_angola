...
//         ini_set('display_errors', 0);
    ...
    public function index()
    {
        ...
        if ( isset($get['uf']) ) { $uf = strtoupper($_GET['uf']); } else { $uf = null; }
    ...
    public function detalhes($cd_cotacao)
    {
        ...
        if ( $this->session->has_userdata('cot_manual') ) { unset( $_SESSION['cot_manual'] ); }
        ...
        $data['integrador'] = ( strpos($cd_cotacao, 'COT') !== false ) ? 'SINTESE' : 'BIONEXO';
            ...
            if ( isset($marca['marcado']) ) {
                ...
                if ( isset($marca['nivel']) ) {
                    ...
                    $novoProduto = [
                        ...
                        "id_produto" => ( $post['integrador'] == 'SINTESE' ) ? intval($post['id_produto_sintese']) : null,
    ...
    public function enviar_resposta()
    {
        ...
        # Criar espelho
        $mirror = $this->COTACAO_MANUAL->createMirror($post, $produtosOrdenados);

        # Criar XML
        $xml = $this->COTACAO_MANUAL->createXML($post, $produtosOrdenados);
     ...
     * @return  view
     */
    public function index_depara($integrador, $codigo_produto, $id_cliente)
    {
     ...
     * @return  json
     */
    public function findProduct($integrador, $codigo_produto, $id_cliente)
    {
        ...
        if ( $integrador == 'SINTESE' ) {
            ...
            if ( isset($produto) && !empty($produto) ) {
                ...
                $output = ['type' => 'success', 'message' => 'Produto encontrado!', 'link' => "{$this->route}index_depara/{$integrador}/{$id_produto}/{$id_cliente}"];
            } else {
    ...
    public function datatable_ocultadas()
    {
        $datatables = $this->datatable->exec(
            ...
            "cot.id_fornecedor = {$this->session->id_fornecedor} AND cot.oculto = 1"
        );
