...
<?php echo $sidebar; ?>
    <div class="content">
        ...
        <div class="accordion mb-3" id="accordionCompradores"></div>
    ...
    <script>
        ...
        function consulta( codigo, min, max, media, preco_catalogo, id_fornecedor ) 
        {
            ...
            $.post(url, data, function( xhr ) { 
                ...
                Object.entries(xhr).forEach(([key, comprador]) => {
                    ...
                    Object.entries(comprador.produtos).forEach(([key2, produto]) => {
                        ...
                        precos += `<td><b>Preço tabela: </b>${produto.preco_catalogo_format}</td>`;
                        
                        precos += `<td class="text-center"><button class="btn btn-light btn--icon-text"><a onclick="consulta(${produto.codigo}, ${produto.min}, ${produto.max}, ${produto.media}, ${produto.preco_catalogo}, ${produto.id_fornecedor})" data-toogle="tooltip" title="Analise de mercado" ><i class="fas fa-volume-up"></i></a></button></td>`;

                        var produto_descricao = ( produto.descricao != '' ) ? `${produto.nome_comercial} - ${produto.descricao}` : produto.nome_comercial;

                        cardProdutos += `
                            <div class="card mb-2">
                                <div class="card-header" style="cursor: pointer;" id="headingPreco${key}_${key2}" data-toggle="collapse" data-target="#collapsePreco${key}_${key2}" aria-expanded="true" aria-controls="collapsePreco${key}_${key2}">
                                    <h6 class="mb-0 text-muted">
                                        COD.: ${produto.codigo} - ${produto_descricao}
                                    </h6>
                                </div>

                                <div id="collapsePreco${key}_${key2}" class="collapse show" aria-labelledby="headingPreco${key}_${key2}" data-parent="#accordionPrecos">
                                    <div class="card-body">
