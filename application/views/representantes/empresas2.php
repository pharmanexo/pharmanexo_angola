        ...
        <div class="col-12 col-lg-6 bg-light formColumn">
            <div class="card text-center shadow " style="background-color: rgba(255,255,255,.7); margin-top: 150px" id="telaLogin">
                <div class="card-body">
                    ...
                    <p>O seu login está cadastrado em mais de um fornecedor, então selecione abaixo a empresa que deseja acessar.</p>
                    <form id="formSelecionarEmpresa" method="post" class="frm" action="<?php echo $frm_action; ?>">
                        <div class="form-group">
                            <select name="empresa" id="empresa" class="form-control">
                                <?php if (isset($empresas)) { ?>
                                    <?php foreach ($empresas as $empresa){ ?>
                                        <option value="<?php echo $empresa['id']; ?>"><?php echo "{$empresa['cnpj']} - ".strtoupper($empresa['razao_social'])." | {$empresa['cidade']}-{$empresa['estado']}"?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </div>
