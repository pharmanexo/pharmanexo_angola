...
class MatchDeParaClientesApoio extends CI_Controller
{
    private $bio;
    private $hmg;
    private $unidades;
    ...
    public function index()
    {
        ...
        $encontrados = $this->db
            ->distinct()
            ->select('*')
            ->from('apoio_produtos_match')
            ->where('visto', 0)
            //->where('id', '1927942')
            //  ->where('codigo_catalogo', '11034')
            // ->where('id_cliente', 15821)
            ->limit(50000)
            ->get()
            ->result_array();
        ...
        if (!empty($encontrados)) {
            ...
            foreach ($encontrados as $jj => $encontrado) {
                ...
                $this->db->where('id', $encontrado['id'])->update('apoio_produtos_match', ['visto' => 1]);
                ...
                $encontrado['descricao_catalogo'] = str_replace(["(", ")", 'INPM', 'inpm', "CMP", "cmp"], ["", "", "%", "%", "COMP", "comp"], $encontrado['descricao_catalogo']);
                ...
                if (empty($principios)) {
                    ...
                    foreach ($keys as $key) {
                        $key = $this->tirarAcentos(str_replace(" ", "", $key));
                ...
                if ($test) {
                    ...
                    $this->db->update('apoio_produtos_match', ['associado' => 1]);
                }
    ...
    public function getProdutosClientes()
    {
        //$id_cliente = 14861;
        $produtos = $this->bio
            ->select('ct.codigo, ct.descricao, ct.unidade, ct.id_cliente')
            ->from('pharmanexo.vw_produtos_clientes_sem_depara_apoio ct')
            ->join('pharmanexo.compradores c', ' c.id = ct.id_cliente')
            //->where("c.estado = 'SP'")
            // ->where('ct.id_categoria', 100)
            // ->where('ct.codigo', 259399)
            //  ->where('c.id', 15813)
            //->where('(ct.process = 0 or ct.process is null)')
            // ->where('(ct.ocultar = 0 or ct.ocultar is null)')
            ->limit(1000)
            ->get()->result_array();
        ...
        foreach ($produtos as $k => $produto) {
            ...
            $produto['descricao'] = $produto['descricao'] . " " . $produto['unidade'];
            ...
            if (isset($keys[0])) {
                // CONSULTA ANVISA
                if (strtolower($keys[0]) != 'cloridrato') {
                    $principios = $this->db
                        ->distinct()
            ...
            if (isset($s)) $s = str_replace("'", '', $s);
            if (isset($s2)) $s2 = str_replace("'", '', $s2);
            ...
            $query = "WITH cte1 AS (SELECT DISTINCT ms.id_produto       as ID_PRODUTO,
                                      ...
                                      from pharmanexo.produtos_marca_sintese ms,
                                           (SELECT distinct c.codigo, concat(c.descricao, ' ', c.unidade) as descricao
                                            FROM cotacoes_apoio.catalogo c
                                            WHERE id_cliente = {$id_cliente} and c.codigo = '{$produto['codigo']}') c,
                                           (SELECT distinct UPPER(SUBSTRING_INDEX(descricao, ' ', 1)) as PRIMEIRO_NOME
                                            FROM cotacoes_apoio.catalogo c
                        ...
                        SELECT *
                        FROM cte1
                        WHERE cte1.DESCRICAO_SINTESE like CONCAT((SELECT distinct UPPER(SUBSTRING_INDEX(descricao, ' ', 1)) as PRIMEIRO_NOME
                                                                  FROM cotacoes_apoio.catalogo c
                                                                  WHERE id_cliente = {$id_cliente}
            ...
            foreach ($t as $item) {
                ...
                $insert = [
                    ...
                    'descricao_sintese' => $item['DESCRICAO_SINTESE'],
                    'palavra_chave' => $item['PALAVRA_CHAVE'],
                    'codigo_catalogo' => $item['CODIGO_CATALOGO'],
                    'descricao_catalogo' => $item['DESCRICAO_CATALOGO'],
                    'id_cliente' => $id_cliente,
                    'principios' => implode('|', $pImplode)
                ];


                $in = $this->db->insert('apoio_produtos_match', $insert);
        ...
        foreach ($naoAchou as $value) {
            ...
            $this->bio->where('id_cliente', $id_cliente);
            $this->bio->update('catalogo', ['ocultar' => 1, 'not_found' => 1, 'process' => 1]);
    ...
    private function process($data)
    {
        ...
        if ($data['mg'] == 0 and $data['ml'] == 0 and $data['mcg'] == 0 and $data['ui'] == 0 && $data['perc'] == 0) {
            return false;
        }
