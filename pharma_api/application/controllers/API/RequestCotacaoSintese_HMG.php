...
class RequestCotacaoSintese_HMG extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->urlCliente = ["http://c7.plataformasintese.com:8080/integrationservice.asmx"];
	...
	public function connectSintese($fornecedor)
	{
		try {
			...
			foreach ($this->urlCliente as $url) {
				...
				$function = 'ObterCotacoesPharmanexo';
				$arguments = array('ObterCotacoesPharmanexo' => array( 'cnpj' => preg_replace("/\D+/", "", $fornecedor['cnpj']), ));

				libxml_disable_entity_loader(false);
				$options = array('location' => $url);
				$result = $client->__soapCall($function, $arguments, $options);

				$resposta = $result->ObterCotacoesPharmanexoResult;
	...
	public function checkRecordSintese()
	{
		...
		if ( $interval->format("%i") > 20 && date('N') < 6 && intval(date('G')) >= 7 && intval(date('G')) < 20 ) {
