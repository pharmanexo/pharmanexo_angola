...
class ProdutosHmg extends MY_Controller
{
...
//        exit();
        ...
        if ($this->input->method() == 'post') {
            ...
            if (isset($getDados)) {
                ...
                foreach ($getDados['produtos'] as $produto) {
                    $salvaNovoEsotque[] =
                        [
                            'codigo' => preg_replace('/[^0-9]/', '', $produto['codigo']),
    ...
    public function cadastrarProduto()
    {
        if ($this->input->method() == 'post') {
            ...
            if ($_SESSION['id_fornecedor'] == 5046){
                $f = fopen("hosp.json", 'w+');
                fwrite($f, $json);
                fclose($f);
            }
            ...
            if (isset($getDados)) {
                ...
                foreach ($getDados['produtos'] as $produto) {
                    ...
                    $result = $this->db->where('codigo', $codigo)->get('produtos_catalogo');
            ...
            if (isset($getDados)) {
                ...
                foreach ($getDados['produtos'] as $produto) {
                    ...
                    if ($_SESSION['id_fornecedor'] == 5039) {
                        $prod = $this->db
                            ->where('id_fornecedor', $_SESSION['id_fornecedor'])
                            ->where('codigo', preg_replace('/[^0-9]/', '', $produto['codigo']))
