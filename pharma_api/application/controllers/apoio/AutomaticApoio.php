...
class AutomaticApoio extends CI_Controller
{
    /**
     * @author : Chule Cabral
     * Data: 01/11/2020
    ...
    private function getProdsCots($params)
    {
        ...
        $result = $this->apoio->where('id_cotacao', $params['id_cotacao'])
    ...
    private function createObject($objCotacao)
    {
        ...
        $post = $objCotacao['dados'];

        $fornecedor = $this->db->where('id', $post['id_fornecedor'])->get('fornecedores')->row_array();
        ...
        $cabecalho->appendChild($dom->createElement("CNPJ_Hospital", mask($post['cnpj_cliente'], '##.###.###/####-##')));
        $cabecalho->appendChild($dom->createElement("Faturamento_Minimo", $post['valor_minimo']));
        ...
        $cabecalho->appendChild($dom->createElement("Id_Forma_Pagamento", $post['forma_pagamento']));
        $cabecalho->appendChild($dom->createElement("Frete", 'CIF'));
        $cabecalho->appendChild($dom->createElement("Custo_Estimado_Frete", ' - '));
        $cabecalho->appendChild($dom->createElement("Observacoes", $post['obsFornecedor']));
        ...
        foreach ($objCotacao['produtos_fornecedor'] as $k => $produto) {

            $this->apoio->where('id_cotacao', $post['id_cotacao']);
            $this->apoio->where('cd_produto_comprador', $produto['cd_produto_comprador']);
            $prod = $this->apoio->get('cotacoes_produtos')->row_array();

            # Percorre as marcas para cada produto
            foreach ($produto['marcas_encontradas'] as $p) {
             ...
             /*  var_dump($p);
                ...
                $item->appendChild($dom->createElement('Codigo_Produto', $prod['cd_produto_comprador']));
                $item->appendChild($dom->createElement('Preco_Unitario', $p['preco_oferta']));
                $item->appendChild($dom->createElement('Preco_Unitario_Liquido', $p['preco_oferta']));
                ...
                $item->appendChild($dom->createElement('Fabricante', utf8_encode(utf8_decode("{$p['marca']}"))));
                $item->appendChild($dom->createElement('Embalagem', 'unidade'));
                $item->appendChild($dom->createElement('Quantidade_Embalagem', $p['qtd_unidade']));
                $item->appendChild($dom->createElement('Comentario', (isset($p['obs_produto']) && !empty($p['obs_produto'])) ? $p['obs_produto'] : ' - '));
                $item->appendChild($dom->createElement('Codigo_Rastreabilidade', $p['codigo']));
                ...
                $extra->appendChild($dom->createElement('Valor', $p['codigo']));
    ...
    public function index()
    {
        ...
        foreach ($getFornecedores as $fornecedor) {
            ...
            $margem_estoque = $this->db->where('id', $fornecedor['id'])
                ->get('fornecedores')
