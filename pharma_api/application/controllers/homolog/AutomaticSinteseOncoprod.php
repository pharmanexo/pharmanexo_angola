...
class AutomaticSinteseOncoprod extends CI_Controller
{
    /**
     * @author : Chule Cabral
     * Data: 01/11/2020
    ...
    public function __construct()
    {
        ...
        $this->load->model('hmg/AutomaticsEngineOncoprod', 'Engine');

        $this->sint = $this->load->database('teste_sintese', true);
    ...
    private function getFornecedores($all = false)
    {

        $fornecedores = $this->db->select('id, margem_estoque, cnpj, config')
            ->where_in('tipo_venda', [2, 3])
            ->where_in('id', [12, 111, 112, 115, 120, 123, 126])
            ->order_by('id')
            ->get('fornecedores')
            ->result_array();
    ...
    private function checkProductSent($params)
    {
        if ($this->configs['checkPrdSent']) {
            ...
            $result = $this->db->where("id_fornecedor in ({$lojas})")
                ->where('cd_cotacao', $params['cd_cotacao'])
    ...
    private function mountArrayProdutos($params, $array)
    {
        return [
    ...
    private function productRestrition($cod)
    {
        $prods = [
            110304,
            347,
    ...
    private function prodsEncontrados($params, $prodsCotacao)
    {
        ...
        foreach ($prodsCotacao as $key => $prod) {
            ...
            $select = "cat.codigo, cat.descricao, cat.apresentacao, cat.unidade,
					   cat.nome_comercial, cat.marca, cat.id_marca, cat.quantidade_unidade, cat.classe, cat.origem";
            ...
            foreach ($resultDepara as $keyProd => $value) {
                ...
                /**
                 * verfica se o clinete é SPI (são paulo interior) e muda o id_estado para 28
                 */
                $cont = $this->Engine->checkClienteSpi($params);
                if (!empty($cont)) {
                    $params['id_estado'] = 28;
                }
                ...
                /**
                 * Verifica se o cliente é contribuinte no banco para aplicar a regra logistica
                 */
                $cont = $this->Engine->checkClienteContribuinte($params);
                if (!empty($cont)) {
                    $params['icms'] = 'CONTRIBUINTE';
                }


                #busca o icms do estado do comprador
                $icmsEstado = $this->Engine->getIcmsEstado($params, $this->configs);
                if (!empty($icmsEstado)) {
                    $params['icms_estado'] = $icmsEstado['icms'];
                }

                $checkProdSend = $this->checkProductSent($params);
                ...
                if (isset($checkEstoque['result']['total']) && intval($checkEstoque['result']['total']) == 0) {
                    $this->logs['PRODS-COT'][$key]['produtos_fornecedor'][$keyProd]
                    ['restricao']['estoque'] = 0;
                }
                ...
                if ($this->productRestrition($params['codigo']) && $params['id_fornecedor'] == 126) {
                    $ignoraMapa = true;
                } elseif ($this->productRestrition($params['codigo']) && $params['id_fornecedor'] != 126) {
                    $this->logs['PRODS-COT'][$key]['produtos_fornecedor'][$keyProd]
                    ['restricao']['restricao'] = 'Cotar apenas por DF';
                    continue;
                }
                ...
                if (!$ignoraMapa) {
                    /**
                     * verifica se o produto tem regra logistica
                     */

                    $prodLog = $this->Engine->checkProdutoLogistica($params);

                    if (empty($prodLog)) {

                        $this->logs['PRODS-COT'][$key]['produtos_fornecedor'][$keyProd]
                        ['restricao']['regra_logistica'] = FALSE;
                    ...
                    if ($prodLog['loja1'] !== '0') {
                        $arrLojas[] = intval($prodLog['loja1']);
                    }

                    if ($prodLog['loja2'] !== '0') {
                        $arrLojas[] = intval($prodLog['loja2']);
                    }
                    ...
                    if (!$checkEstoque['status']) {
                        ...
                        foreach ($arrLojas as $elem) {

                            $params['id_fornecedor'] = $elem;

                            $arrFornecedores = $this->getFornecedores(true);

                            $keyNewFornecedor = array_search(strval($elem), array_column($arrFornecedores['result'], 'id'));

                            $params['cnpj_fornecedor'] = $arrFornecedores['result'][$keyNewFornecedor]['cnpj'];

                            $_checkEstoque = $this->Engine->getEstoque($params, $this->configs);

                            if ($_checkEstoque['status']) {

                                $checkEstoque = $_checkEstoque;

                                $regra_logistica = TRUE;
                                break;
                ...
                $arr =
                    [
                        ...
                        'desconto_vendaDif' => isset($params['desconto_percentual']) ? $params['desconto_percentual'] : 0,
            ...
            if (empty($newArr) && empty($newArrLogic))
                continue;

            if ($regra_logistica) {
                $produtos[$params['id_fornecedor']]['dados'] = $params;
                $produtos[$params['id_fornecedor']]['produtos_fornecedor'][] = $this->mountArrayProdutos($params, $newArrLogic);
            } else {
                $produtos[$params['id_fornecedor']]['dados'] = $params;
                $produtos[$params['id_fornecedor']]['produtos_fornecedor'][] = $this->mountArrayProdutos($params, $newArr);
            }
        }
    ...
    private function createObject($objCotacao)
    {
        ...
        foreach ($objCotacao['produtos_fornecedor'] as $prodsOferta) {
            ...
            foreach ($prodsOferta['marcas_encontradas'] as $prodsMarca) {
                ...
                $marca->appendChild($dom->createElement("Ds_Marca", htmlspecialchars($prodsMarca['marca'])));
        ...
        /* $f = fopen(preg_replace("/\D+/", "", $objCotacao['dados']['cnpj_fornecedor']) . ".xml", "w+");
    ...
    public function index()
    {
        ...
        foreach ($getFornecedores['result'] as $fornecedor) {
            ...
            $estados = $this->getEstados($fornecedor['id']);
            $clientes = $this->getCompradores($fornecedor['id']);
            ...
            foreach ($getCotacoes['result'] as $cotacao) {
                ...
                //   verifica se ja foi respondido no manual e pula a cotação
                $checkSendCot = $this->Engine->getSendCot($cotacao['cd_cotacao']);
                if (intval($checkSendCot) > 0) {
                    $this->Engine->saveLogs($arr['CONFIGS-COT'] =
                        [
                            'cotacaoRespondidaManual' => TRUE
                        ], $params, $this->configs);
                    continue;
                }
                ...
                foreach ($dataCot as $data) {

                    $createObject = $this->createObject($data);
