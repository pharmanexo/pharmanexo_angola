...
class AutomaticsEngineOncoprod extends CI_Model
{
    private $bio;
    private $mix;
    ...
    public function __construct()
    {
        ...
        $this->bio = $this->load->database('teste_bionexo', true);
        $this->mix = $this->load->database('teste_mix', true);

    }

    #verifica loja prioritaria
    public function lojaPrioridade($params){
        $this->db->where('codigo', $params['codigo']);
        $this->db->where('id_loja', $params['id_fornecedor']);
        # $this->db->where('id_estado', $params['id_estado']);

        $data = $this->db->get('loja_prioridade')->row_array();

        return $data;
    }

    # verifica a regra logistitica
    public function checkProdutoLogistica($params)
    {

        $this->db->where('icms', $params['icms']);
        $this->db->where('id_fornecedor', $params['id_fornecedor']);
        $this->db->where('id_estado', $params['id_estado']);
        $this->db->where('classe', $params['classe']);
        if ($params['icms'] == 'ISENTO' && isset($params['origem'])) {
            $this->db->where('origem', $params['origem']);
        }

        $data = $this->db->get('mapa_logistico')->row_array();
    ...
    # verifica a regra logistitica
    public function checkClienteSpi($params)
    {

        $this->db->where('id', $params['id_cliente']);
        $this->db->where('spi', '1');
    ...
    public function checkClienteContribuinte($params)
    {

        $this->db->where('id_cliente', $params['id_cliente']);
        $this->db->where('id_fornecedor', $params['id_fornecedor']);

        return $data = $this->db->get('clientes_contribuintes')->row_array();
    }

    #verifica o icms do estado do cliente
    public function getIcmsEstado($params)
    {

        $this->db->where('id_estado', $params['id_estado']);

        return $data = $this->db->get('estado_icms')->row_array();
    }

    #verifica se a cotação tem oferta manual
    public function getSendCot($cd_cotacao)
    {
        $this->db->where('id_fornecedor in (12,111,112,115,120,123,126)');
        $this->db->where('cd_cotacao', $cd_cotacao);
        $this->db->where('nivel', 1);

        $dt = $this->db->get('cotacoes_produtos')->result_array();
    ...
    public function getCotsFornecedor($params)
    {
        ...
        if (isset($params['revisadas']) && $params['revisadas'] == false){
            $where .= " and revisao <> 0";
        }
    ...
    public function enabledAutomatic($params, $configs)
    {
        ...
        if ($configs['checkEnabledAuto']) {
            ...
            if (isset($configs['IdIntegrador'])) {
                $this->db->where('integrador', $configs['IdIntegrador']);
            }

            $this->db->where('id_fornecedor', $params['id_fornecedor']);
            $this->db->where_in('regra_venda', [1, 2]);
            $this->db->group_start();
            $this->db->where('id_cliente', $params['id_cliente']);
            $this->db->or_where('id_estado', $params['id_estado']);
            $this->db->group_end();
            $this->db->limit(1);
            $result = $this->db->get('controle_cotacoes');


            if (IS_NULL($result->result_array()))
                return FAlSE;
                ...
                [
                    ...
                    'result' =>
                        [
                            ...
                            'validade' => date('d/m/Y', strtotime(str_replace('-', '/', $result['validade'])))
                        ]
                ];
    ...
    public function getPriceProd($params, $configs)
    {
        ...
        if ($valuePrice == 0) {
            ...
            $tablePrice = "produtos_preco_oncoprod";
        ...
        $desconto_percentual = $params['desconto_percentual'] ?? 0;
        ...
        foreach ($params['prodsEspelho'] as $produto) {
            ...
            foreach ($produto['marcas_encontradas'] as $item) {
                ...
                $row .= "
                                ...
                                <tr>
                                    <td colspan='6' style='border: 1px solid #dddddd; text-align: left; padding-right: 20px'>Observações: {$item['nome_comercial']}</td>
                                </tr>
                            ";
    ...
    public function submitEmail($params, $configs)
    {
        ...
        /**
         * Responspavel por pegar todos os e-mails responspaveis de cada empresa.
         */
            ...
            if (!IS_NULL($notify) && !empty($notify)) {
                ...
                $mail =
                    [
                        ...
                        "assunto" => "TESTE - COTACAO {$configs['integrador']} #{$params['dadosCotacao']['cd_cotacao']} {$ds_cliente} - {$params['dadosCotacao']['uf_cotacao']} {$dt_cotacao}",
